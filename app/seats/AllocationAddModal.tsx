...
export default function AllocationAddModal({
  ...
  function generateCustomShiftPairs(baseList: string[], rotateRight: boolean) {
    ...
    for (let i = 0; i < fullList.length; i += 2) {
      const first = fullList[i] || "빈자리";
      const second = fullList[i + 1] || "빈자리";

      // 첫 번째 자리가 빈자리이고 두 번째 자리에 사람이 있는 경우 앞뒤를 맞바꿔 사람이 앞자리로 오도록 함
      if (first === "빈자리" && second !== "빈자리") {
  ...
  const handleLoadLastRoundFromDB = async () => {
    ...
    try {
      ...
      const result = await getSeatsDataByRounds([latestRound.id]);
      if (!result.success) {
        alert(`기존 그룹 조회 실패: ${result.message}`);
        return;
      }
      ...
      const latestRoundSeats = groups.reduce(
        (
          members: string[],
          group: { member_1?: string; member_2?: string }
        ) => {
      ...
      if (latestRoundSeats.length > 0) {
        ...
        alert(
          `최신(${latestRound.roundNumber}회차) 데이터를 성공적으로 불러왔습니다!`
        );
      }
    } catch (err: any) {
  ...
  for (let i = 0; i < flatMembers.length; i += 2) {
    ...
    flatPairs.push([
      m1
        ? { ...m1, flatIndex: i }
        : { name: "빈자리", status: false, flatIndex: -1 },
      m2
        ? { ...m2, flatIndex: i + 1 }
        : { name: "빈자리", status: false, flatIndex: -1 }
    ]);
      ...
      <div className="bg-white w-full max-w-xl rounded-2xl p-6 space-y-4 shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between border-b pb-3 border-slate-100">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => setNumberPerGroup((prev) => (prev === 3 ? 2 : 3))}
            ...
            >
              ...
              <span>
                {numberPerGroup === 3 ? "2인 배치로 전환" : "3인 배치로 전환"}
              </span>
            </button>
